defmodule MusicDB.Exercises.QueryExercises do
  ...
  def find_artist_by_name(name) do
    ...
    # Then use Repo.all to fetch the record with that query.
    query = from a in "artists", where: a.name == ^name, select: [:name]
    Repo.all(query)
  end
  ...
  def search_tracks(title) do
    ...
    query = from a in "tracks", where: like(a.title, ^title), select: [:title]
    Repo.all(query)
  end
  ...
  def order_artists_by_name do
    ...
    query = from a in "artists", select: a.name, order_by: [desc: :name]
    Repo.all(query)
  end
